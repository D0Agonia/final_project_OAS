...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit_button"])){
    try{
        ...
        elseif(filter_var($email, FILTER_VALIDATE_EMAIL) == true && filterEmail($email) == true){
            $sql = "SELECT requestPasswordChange('$email') AS STATUS";
            $queryResult = $conn->query($sql); $row = $queryResult->fetch_assoc();
            $requestResult = $row['STATUS'];
        }
        ...
        elseif(strlen($requestResult) == 32){
            $show_error = false;

            $postData = ['code' => $requestResult];
            $url = 'localhost/final_project_OAS/change_pass.php' . http_build_query($postData);
            $message = "
            <!DOCTYPE html>
            <body>
